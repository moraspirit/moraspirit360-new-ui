...
import styles from '../app/Home/Home.module.css'; // Adjust the import path as necessary
...
const Home = () => {
  ...
  <div className={styles.heroContainer}>
    ...
    <div className={styles.darkOverlay}></div> {/* Dark overlay */}
...
<section className={`${styles.thirdSection} ${styles.scrollUpAnimation}`}>
  <h1>WHAT WE DO BEST</h1>
      ...
      {/* Past Events Section */}
      <section className="w-full flex flex-col items-center justify-start gap-2">
        ...
        <div className="w-[80%] mt-10 sm:mt-5 flex flex-col sm:flex-row sm:gap-5 gap-10 flex-1 items-center justify-center">
          {pastEvents.map((event, index) => (
            <EventCard key={index} delay={index * 200} imgUrl={event.url} title={event.title} description={event.description} hoverText={true} cardStyle={styles.eventCard} />
          ))}
        </div>
      </section>
      <Footer></Footer>
